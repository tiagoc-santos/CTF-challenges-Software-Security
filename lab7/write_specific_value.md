...
- Where: Where is the vulnerability present
  - Line 15 in the `04_match_value.c` file
...
3. By sending `p32(target_address) + b'%322x.%7$n'` as input, will put the address of `target` at the beginning of the `buffer` and then use the `%n` format specifier to write the number of bytes written so far (which is 327, 4 from the address + 322 from the 1st register below the format string + 1 from '.') into that address, changing the value of `target` to 327, printing the flag.
